...
import {
  ...
  type ResolvedTrickPrompt,
  type SupportedTrickSlug,
} from "./common-system-v1";
...
export const version = "common-system-v2";
export const promptVersionFamily = "v2" as const;
...
- confidenceは**0以上1以下の小数**にしてください。scoresの0〜100とは尺度が異なります。確信が高い場合は0.9、五分五分なら0.5のように表してください。1を超える値やパーセント表記（90など）は使用しないでください。
...
export function getPromptForTrick(trickSlug: string): ResolvedTrickPrompt {
  ...
  return composePrompt({ version, prompt }, resolveTrickPrompt(trickSlug));
}
