...
S3とBedrockの呼び出し元は同じソウルに置く。S3 URIで動画を渡すため、バケットはBedrockの呼び出し元リージョンと同一にする。

当初はTwelveLabs PegasusのAPAC提供がソウルだけだったためソウルを選んだ。17本の品質比較でNova 2 Liteを既定に決定した後、`jp.amazon.nova-2-lite-v1:0`で動画処理を日本国内に限定するため東京移行を試みたが、このAWSアカウントでは東京のBedrockクォータ引き上げが承認されず、東京の代替Novaモデルにも利用枠がなかった。そのため、クォータを利用できるソウルの`global.amazon.nova-2-lite-v1:0`へ戻す。

> **本番有効化前の重要事項:** 現在の`tricksight-vercel-role`にはNovaを呼ぶ`bedrock:InvokeModel`が付与されていない。ソウルバケットへのS3権限と、下記のGlobal推論プロファイル権限を追加するまで、本番で`bedrock-nova`は動作しない。

Global推論プロファイルはAPAC外を含むAWS商用リージョンへ動画を処理のため転送し得る。これは依頼者本人の動画だけを扱うMVP検証段階に限定した受容リスクである。第三者ユーザーの動画を受け入れる前にデータ所在地を再検討し、東京クォータが得られた場合はJP推論プロファイルへ移行する。
...
次の権限ポリシーを`tricksight-vercel-runtime-policy`として更新し、このロールだけにアタッチする。S3のARNは利用するソウルバケットを明示する。
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::tricksight-dev-561143850472-ap-northeast-2-an/private/*"
    },
    {
      "Sid": "InvokeNovaGlobalInferenceProfile",
      "Effect": "Allow",
      "Action": "bedrock:InvokeModel",
      "Resource": "arn:aws:bedrock:ap-northeast-2:561143850472:inference-profile/global.amazon.nova-2-lite-v1:0",
      "Condition": {
        "StringEquals": {
    ...
    {
      "Sid": "InvokeNovaGlobalInRegionModel",
      "Effect": "Allow",
      "Action": "bedrock:InvokeModel",
      "Resource": "arn:aws:bedrock:ap-northeast-2::foundation-model/amazon.nova-2-lite-v1:0",
      "Condition": {
        "StringEquals": {
          "aws:RequestedRegion": "ap-northeast-2",
          "bedrock:InferenceProfileArn": "arn:aws:bedrock:ap-northeast-2:561143850472:inference-profile/global.amazon.nova-2-lite-v1:0"
        }
      }
    },
    {
      "Sid": "InvokeNovaGlobalModel",
      "Effect": "Allow",
      "Action": "bedrock:InvokeModel",
...
参考：AWS公式の[Globalクロスリージョン推論のIAM要件](https://docs.aws.amazon.com/bedrock/latest/userguide/global-cross-region-inference.html)と[Nova 2 Liteモデルカード](https://docs.aws.amazon.com/bedrock/latest/userguide/model-card-amazon-nova-2-lite.html)。
...
S3_BUCKET_NAME=tricksight-dev-561143850472-ap-northeast-2-an
...
**`AWS_REGION`の明示は必須である。** 現在クォータを利用できるGlobal推論プロファイルの呼び出し元と動画S3はソウル（`ap-northeast-2`）にそろえる。将来JP推論へ切り替える場合、コードはJPプロファイルと東京以外のリージョンの組み合わせを起動時に拒否する。
...
`global.amazon.nova-2-lite-v1:0`はAPAC外を含むAWS商用リージョンへ処理をルーティングし得る。Productionで第三者の動画を受け入れる前に、データ所在地要件と東京JP推論への再移行可否を確認する。
...
- バケット名：`tricksight-dev-561143850472-ap-northeast-2-an`
- パブリックアクセスはすべてブロックする
...
`tricksight-dev-561143850472-ap-northeast-1-an`を既に作成している場合は、将来のJP推論移行用に**空のまま残す**ことを推奨する。バケット名を保持でき、東京クォータが得られた際の再作成も不要になる。オブジェクトがあれば削除し、パブリックアクセスブロックは維持する。現在のVercel OIDCロールには東京バケットの権限を付けず、`S3_BUCKET_NAME`からも参照しない。不要資源を一切残さない方針を優先する場合は削除してもよいが、将来同じグローバル一意名を再取得できる保証はない。

## 5. モデルアクセスを有効化する（ソウルリージョン）

Bedrockコンソールを**ソウル（ap-northeast-2）**に切り替え、Nova 2 Liteが利用可能であることを確認する。

アカウントレベルの設定なので、管理者ログインのまま行う。

- **Amazon Nova 2 Lite**：`global.amazon.nova-2-lite-v1:0`を使用する。ソースはソウル、処理先は対応するAWS商用リージョン全体である。
...
IAM更新が完了した後にこの確認を行う。特に以下を確認する。

- `global.amazon.nova-2-lite-v1:0`が一覧にあり、ソウルをソースとして使えること
- 実行ロールのポリシーにソウルの推論プロファイル、ソウルの基盤モデル、Global基盤モデルARNがすべて含まれること

T10-revertでは実API呼び出しや評価の再実行は行わない。leaderが別途`pnpm verify:aws`でソウルのGlobal推論プロファイルを確認済みである。
