...
状態: **設計案。leader承認前のため製品コード・DB・画面には未適用**
...
同条件の前回日付と詳細ページへのリンクを示し、次の3項目を表示する。

1. 膝の曲がり: 左右膝角度平均の最小値。角度が小さいほど深く曲がっている
2. 腰の上下動: 腰中心のp90-p10を胴長で正規化した値。絶対的な跳躍高ではない
3. 着地時の体幹傾斜: 鉛直からの角度。大きい・小さいだけで良否を決めない

値は小数1桁で表示する。色で良し悪しを付けず、「5.2°深く曲がった」「傾きが2.0°大きい」のように変化だけを記述する。WebKit / Chromium差を踏まえ、膝と体幹は2°以内、腰上下は0.02胴長以内を暫定的に「ほぼ同じ」とする。この許容幅は実機比較後に確定し、処理版へ含める。
...
| ID | 内容 | 完了条件 |
| --- | --- | --- |
| T11-3 | ブラウザ骨格コア | 製品用Workerが固定10fps、3指標、品質ゲート、進捗、cancel、timeoutを返す。生33点はWorker外へ出さない。Chromium / WebKitのMP4・MOV integration testが通る |
...
T11-3以降はleaderがこの設計を承認してから着手する。骨格値のLLM投入はこの分割に含めず、独立した評価タスクへ延期する。
