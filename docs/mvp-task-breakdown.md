...
| ID | タスク | 完了条件 | 状態 |
| --- | --- | --- | --- |
| T11-1 | ブラウザ版Pose Landmarkerの実現可能性を同一評価動画で検証する。具体的なサンプルレート、対象端末、測定項目、合格条件は次の依頼で確定する | 処理時間、骨格検出率、反復再現性、端末・ブラウザ差、MP4/MOVデコード可否、CPU/GPU差を実測し、ブラウザ採否を判断できる | **未着手（最優先）** |
